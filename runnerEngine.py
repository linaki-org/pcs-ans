...
class Game:
    ...
    def goRoom(self, id, resetOffset=True):
        ...
        self.canvas.create_image(0-self.actualRoom["xOffset"], 0-self.actualRoom["yOffset"], anchor=NW, image=self.actualRoom["image"])
        self.actualObjects={}
        ...
        for object in objects:
            ...
            if files[0]:
                ...
                obId=self.canvas.create_image(x-self.actualRoom["xOffset"], y-self.actualRoom["yOffset"], image=self.actualObjects[object], anchor=NW)
                self.canvas.tag_bind(obId, "<Button-1>", self.genObjectHandler(object))
            else:
                self.actualObjects[object]=None
                obId=self.canvas.create_rectangle(x-self.actualRoom["xOffset"], y-self.actualRoom["yOffset"], x-self.actualRoom["xOffset"]+width, y-self.actualRoom["yOffset"]+height, fill="gray", outline="", stipple="@transparent.xbm")
                self.canvas.tag_bind(obId, "<Button-1>", self.genObjectHandler(object))
            ...
            for option in options.split(";"):
                if option=="autoTrig":
                    ...
                    if "use "+object not in self.when:
                        ...
                        self.when["use "+object]+=("setObjImg %s 1;sleep 0.5;"%object)+self.when["use "+object]
    ...
    def addOffset(self, xOffset, yOffset=0):
        ...
        if not ((self.actualRoom["yOffset"] + yOffset <=-1) or (self.actualRoom["yOffset"] + yOffset + self.HEIGHT >= self.actualRoom["image"].height())):
    ...
    def setObjImg(self, obj, id):
        ...
        obId=self.canvas.create_image(x-self.actualRoom["xOffset"], y-self.actualRoom["yOffset"], image=self.actualObjects[obj], anchor=NW)
    ...
    def animationStep(self, animationId, objId, imgList, delay, pos=0):
        ...
        self.animations[animationId]=self.tk.after(delay, lambda: self.animationStep(animationId, objId, imgList, delay, pos+1))
